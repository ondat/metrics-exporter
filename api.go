...
func GetOndatVolumesAPI(log *zap.SugaredLogger, apiSecretsPath string) ([]VolumePVC, error) {
	username, password, err := ReadCredsFromMountedSecret(apiSecretsPath)
	if err != nil {
		log.Errorw("failed to read creds", "error", err)
		return nil, err
	}

	c, err := NewOndatHttpClient(username, password, OndatK8sServiceEndpoint)
	if err != nil {
		log.Errorw("failed to create api client", "error", err)
		return nil, err
	}
	...
	for _, ns := range namespaces {
		...
		if err != nil {
			log.Errorw("failed to get volumes in namespace", "namespace", ns.Name, "error", err)
			continue
		}
