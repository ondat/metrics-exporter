...
func TestParseOndatVolumeState(t *testing.T) {
	tests := []struct {
		name string

		inputVolume      *Volume
		inputJsonContent []byte
	...
	}{
		{
			...
			inputJsonContent: []byte(`{"labels":{"csi.storage.k8s.io/pvc/name":"pvc-1-replica"}}`),
			expectedVolume: &Volume{
				Major: 8,
				Minor: 32,
				ID:    "78e88095-e690-49be-b0f3-3f735ef084a5",
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			err := parseOndatVolumeState(tt.inputVolume, tt.inputJsonContent)
			if err != nil {
				require.EqualError(t, tt.expectedErr, err.Error())
			}

			// call to parseOndatVolumeState updates the var `inputVolume`'s reference
			require.EqualValues(t, tt.expectedVolume, tt.inputVolume, "unexpected volumes returned")
		})
	}
}
